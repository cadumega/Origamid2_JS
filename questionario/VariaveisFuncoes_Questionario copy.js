...
let idade = 30
let nomeCompleto2 = `${nome} ${sobreNome} terá ${idade} anos.`;
...
let umNumeroPequeno = 1;
let umNumeroGrande = 9;
...
console.log(calcMultiplicar(2,3));
...
function triplo(resultado) {
    return resultado * 3
}

function triploDaSoma(numero1,numero2) {
    let somar = numero1 + numero2                  // terminei essa parte, criei outra funcao para multiplicar
    return triplo(somar)                            // terminado a fc, coloco para fc retornar o triplo do meu varivel resultado somar. 
}
...
function escreverCartao (titulo, nome, sobrenome) {
    ...
    var informacoes =`${titulo} ${nome} ${sobrenome}`;               // Melhor desse jeito 
    return informacoes;                                              // retorno de informacoes com os dados que foram armazenados
}

console.log(escreverCartao('Dra.', 'Ana', 'Lima'));
console.log(escreverCartao('Dr.', 'Julio', 'Pedra'));
...
// var arrendondado = parseFloat(gerarProbabilidade.toFixed(2));

console.log(gerarProbabilidade(0.5).toFixed(2));                           // acrescentei o metodo toFixed() de arrendondar 2 casas decimais
